...
const char HTTP_SVG_D3JS_LIBRARY[] PROGMEM = "";
...
<h2 style='color:#FFB000;margin:0 0 4px;font-size:20px;'>MiniWX Station &mdash; Sensor History</h2>\
<p style='color:#888;font-size:13px;margin:0 0 8px;'>Sampling interval: <b style='color:#FFB000;'>2.5 min</b> &bull; Max <b style='color:#FFB000;'>96 samples</b> per chart (~4 h of data) &bull; Right edge = most recent reading &bull; Y-axis auto-scales to data range &bull; X-axis shows elapsed time.</p>\
</div>\
<div>\
...
svg{background:#000;display:block;margin:4px 0;}\
</style>";
...
function drawChart(id,vals,color,unit,fmt){\
  var svg=document.getElementById(id);\
  if(!svg)return;\
  if(!vals||vals.length<2){\
    var m=mk('text',{x:'380',y:'120','text-anchor':'middle',fill:'#555','font-size':'15'});\
    m.textContent='No data yet (collecting, one sample every 2.5 min)';\
    svg.appendChild(m);return;\
  }\
  var W=760,H=240,pL=58,pR=14,pT=12,pB=36,cW=W-pL-pR,cH=H-pT-pB,n=vals.length;\
  var mn=Math.min.apply(null,vals),mx=Math.max.apply(null,vals);\
  if(mn===mx){mn-=1;mx+=1;}\
  var pd=(mx-mn)*0.08;mn-=pd;mx+=pd;\
  function sx(i){return pL+(i/(n-1))*cW;}\
  function sy(v){return pT+(1-(v-mn)/(mx-mn))*cH;}\
  svg.appendChild(mk('rect',{x:pL,y:pT,width:cW,height:cH,fill:'#111'}));\
  for(var i=0;i<=5;i++){\
    var v=mn+(mx-mn)*i/5,y=sy(v);\
    svg.appendChild(mk('line',{x1:pL,x2:pL+cW,y1:y,y2:y,stroke:'#252525','stroke-dasharray':'4,4'}));\
    svg.appendChild(mk('line',{x1:pL-5,x2:pL,y1:y,y2:y,stroke:'#666'}));\
    var lb=mk('text',{x:pL-7,y:y+4,'text-anchor':'end','font-size':'11',fill:'#999'});\
    lb.textContent=(i===5?fmt(v)+' '+unit:fmt(v));\
    svg.appendChild(lb);\
  }\
  svg.appendChild(mk('line',{x1:pL,x2:pL,y1:pT,y2:pT+cH,stroke:'#555'}));\
  ...
  var lv=vals[n-1],lx=sx(n-1),ly=sy(lv);\
  svg.appendChild(mk('circle',{cx:lx,cy:ly,r:'4',fill:color}));\
  var lt=mk('text',{x:Math.min(lx-4,pL+cW-6),y:ly-9,'text-anchor':'end','font-size':'12',fill:color,'font-weight':'bold'});\
  lt.textContent=fmt(lv)+' '+unit;\
  svg.appendChild(lt);\
  svg.appendChild(mk('rect',{x:pL,y:pT,width:cW,height:cH,fill:'none',stroke:'#444'}));\
  var xt=Math.max(1,Math.min(5,n-1));for(var xi=0;xi<=xt;xi++){\
    var si=Math.round((xi/xt)*(n-1));\
    var xp=sx(si);\
    var sec=(n-1-si)*150,ma=Math.round(sec/60);\
    var xl;\
    if(ma===0){xl='now';}\
    else if(ma>=60){var h=Math.floor(ma/60),mm=ma%60;xl=h+'h'+(mm?' '+mm+'m':'')+' ago';}\
    else{xl=ma+'m ago';}\
    svg.appendChild(mk('line',{x1:xp,x2:xp,y1:pT+cH,y2:pT+cH+5,stroke:'#555'}));\
    var xtl=mk('text',{x:xp,y:pT+cH+16,'text-anchor':'middle','font-size':'10',fill:'#666'});\
    xtl.textContent=xl;\
    svg.appendChild(xtl);\
  }\
}\
...
const char HTTP_EXIT_BUTN[] PROGMEM = "<form>\
  <button type='button' onclick='window.history.go(-1);' style='background:#1f232a;color:#e8e8e8;border:1px solid #2a2f37;border-radius:8px;padding:9px 18px;font-size:14px;cursor:pointer'>{{exit_btn}}</button>\
